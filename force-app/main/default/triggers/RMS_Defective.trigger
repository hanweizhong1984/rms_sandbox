trigger RMS_Defective on RTV_Defective__c (before insert,after insert,before update) 
{
    ...
    //开始时间，结束时间(DEF Order)
    Date DEFOrderStartday;
    Date DEFOrderEndday;
    //开始时间，结束时间(DEF Order)
    Date TBOrderStartday;
    Date TBOrderEndday;

    String DEFOrdersyear;
    String DEFOrdersmonth;
    String DEFOrdersday;

    String DEFOrdereyear;
    String DEFOrderemonth;
    String DEFOrdereday;

    String TBOrdersyear;
    String TBOrdersmonth;
    String TBOrdersday;

    String TBOrdereyear;
    String TBOrderemonth;
    String TBOrdereday;
    ...
    {
        //重新设定时间范围（DEF Order）
        DEFOrdersyear = defective.Start_Date__c.addMonths(1).year()+'';
        DEFOrdersmonth = defective.Start_Date__c.addMonths(1).month()+'';
        DEFOrdersday = '20';
        DEFOrderStartday=Date.valueOf(DEFOrdersyear+'-'+DEFOrdersmonth+'-'+DEFOrdersday);
        DEFOrdereyear = defective.Start_Date__c.addMonths(2).year()+'';
        DEFOrderemonth = defective.Start_Date__c.addMonths(2).month()+'';
        DEFOrdereday = '20';
        DEFOrderEndday = Date.valueOf(DEFOrdereyear+'-'+DEFOrderemonth+'-'+DEFOrdereday);

        //重新设定时间范围（TB Order）
        TBOrdersyear = defective.Start_Date__c.addMonths(-1).year()+'';
        TBOrdersmonth = defective.Start_Date__c.addMonths(-1).month()+'';
        TBOrdersday = '15';
        TBOrderStartday=Date.valueOf(TBOrdersyear+'-'+TBOrdersmonth+'-'+TBOrdersday);
        TBOrdereyear = defective.Start_Date__c.year()+'';
        TBOrderemonth = defective.Start_Date__c.month()+'';
        TBOrdereday = '14';
        TBOrderEndday = Date.valueOf(TBOrdereyear+'-'+TBOrderemonth+'-'+TBOrdereday);
        ...
        List<RTV_Order__c> orderList = [
            ...
            Actual_Date_Of_Inspected__c,Actual_Date_Of_WSL_Confirmed__c ,Actual_Date_Of_WSL_Inbound__c,
        ...
        for(RTV_Order__c order:orderList)
        {
            ...
            {
                if(order.Actual_Date_Of_Inspected__c >= DEFOrderStartday && order.Actual_Date_Of_Inspected__c <= DEFOrderEndday)
                {
                    RTV_Order__c newOrder = new RTV_Order__c();
                    newOrder.id = order.id;
            ...
            if(order.RTV_DEF_Summary__c != null && order.From_TakeBack_Order__c != null)
            {
                if(order.Actual_Date_Of_WSL_Inbound__c >= TBOrderStartday && order.Actual_Date_Of_WSL_Inbound__c <= TBOrderEndday)
                {
                    RTV_Order__c newOrder = new RTV_Order__c();
                    newOrder.id = order.id;
                    if(defective.Type__c == 'FTW' && order.Off_Policy_FW__c == false)
